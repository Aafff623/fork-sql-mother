...
完全运行在浏览器中的闯关式 SQL 学习与自由练习应用。维护目标是让上游能力可持续二开，同时保持来源、许可证和行为边界清晰。

## 开始任务前

1. 阅读 `CONTEXT.md` 与 `docs/agents/domain.md`。
2. 用 `git status --short` 确认工作区；用户改动不得覆盖。
3. 定位产品层根：src/core/ SQL 执行和状态；src/levels/ 教学关卡；src/components/ 交互；public/ 静态资源。
...
- README 预览：在仓库根运行 `python -m http.server 4311`，打开 `http://127.0.0.1:4311/preview-readme.html`。
